...
for (forecasting_horizon in c(2, 8, 12)) {
  ...
  for (model_option in c("lc", "apc", "rh")) {
    ...
    for (ix in prediction_horizon) {
      ...
      list_of_extra_exposures[[1]] <- list(Dxt = D2, Ext = E2)
      ...
      muxt_actual_1_val <- G1.mx$rate[[series_code]][as.character(ages.fit), as.character(years.validation)]
      muxt_actual_2_val <- G2.mx$rate[[series_code_2]][as.character(ages.fit), as.character(years.validation)]
      ...
      for (i in 1:N_groups) {
        ...
        mse_0_ix_val <- mse_0_ix_val + (abs(get(
          paste0("muxt_actual_", i, "_val")
        ) - muxt_hat_predicted_val)) #/ get(paste0("muxt_actual_", i, "_val"))
      ...
      for (i in 1:N_groups) {
        mse_1_ix_val <- mse_1_ix_val + (abs(get(
          paste0("muxt_actual_", i, "_val")
        ) - get(paste0(
          "muhat", i, "_val"
        )))) #/ get(paste0("muxt_actual_", i, "_val"))
      ...
      for (i in 1:N_groups) {
        ...
        mse_2_ix_val <- mse_2_ix_val + (abs(get(
          paste0("muxt_actual_", i, "_val")
        ) - get(
          paste0("mm_forecast_", i, "_val")
        ))) #/ get(paste0("muxt_actual_", i, "_val"))
    ...
    out_model <- data.frame(
      ...
      mse_0_val = c(sum(mse_0_val) / (
        length(ages.fit) * val_years * N_groups * length(prediction_horizon)
      )),
      mse_0 = c(sum(mse_0) / (
        length(ages.fit) * N_groups * length(prediction_horizon)
      )),
      mse_1_val = c(sum(mse_1_val) / (
        length(ages.fit) * val_years * N_groups * length(prediction_horizon)
      )),
      mse_1 = sum(mse_1) / (length(ages.fit) * N_groups * length(prediction_horizon)),
      mse_2_val = c(sum(mse_2_val) / (
        length(ages.fit) * val_years * N_groups * length(prediction_horizon)
      )),
...
fwrite(out,
       "C:\\Users\\gpitt\\Documents\\Postdoc\\Torino\\Mortality\\results\\gender_validation_testing_l2.csv")


long_data_val <- pivot_longer(out,
...
out_selected <-   left_join(long_data_val,
                            long_data,
                            by = c("forecasting_horizon", "model", "family"))


fwrite(out_selected,
       "C:\\Users\\gpitt\\Documents\\Postdoc\\Torino\\Mortality\\results\\gender_validation_testing_selection_l2.csv")
