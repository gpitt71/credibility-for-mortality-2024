...
age.model = StMoMo::StMoMo(link="log",
                   staticAgeFun = TRUE,
                   periodAgeFun = NULL,
                   cohortAgeFun = NULL)

AModelfit <- fit(age.model, 
             data = datahat,
             years.fit = years.fit,
             ages.fit = ages.fit)
...
AModelfor <- forecast(AModelfit, h = fcst.horizon)
...
Dxt.hat <-  Ext.actual*AModelfor$rates
...
# AModelfit1 <- fit(age.model, 
#               data = USAdata,
#               years.fit = years.fit,
#               ages.fit = ages.fit)
# F_1==exp(AModelfit1$ax)
...
credible.mu.hat.1 <- AModelfor$rates+alpha1.mx*(F_1_mx-AModelfor$rates)
credible.mu.hat.2 <- AModelfor$rates+alpha2.mx*(F_2_mx-AModelfor$rates)
...
AModelfit2 <- fit(age.model,
              data = SWEdata,
              years.fit = years.fit,
              ages.fit = ages.fit)

AModelfor1 <- forecast(AModelfit1, h = fcst.horizon)
AModelfor2 <- forecast(AModelfit2, h = fcst.horizon)


Dxt.hat.sep <-  E1.actual*AModelfor1$rates+E2.actual*AModelfor2$rates
