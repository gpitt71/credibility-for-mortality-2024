...
# years_ix<-1
# Simulate data: scenario 0 ----

out <- NULL

for (seed_ix in 1) { #1:3
  dt_0 <- data_generator_0(seed = seed_ix)
  ...
  for (forecasting_horizon in c(1,5,8)) {
    ...
    for (years_ix in 1:6) {
      ...
      rm(data_pp,mortality_models_fit,total_model,credibility_model,separate_model)
      
      data_pp <- data_preprocessing_scenario_0(
        data = dt_0,
        N_groups = N_groups,
        ages_fit = ages_fit,
        years_fit = years_fit
...
data.table::fwrite(out,"C:\\Users\\pwt887\\Documents\\GitHub\\credibility-for-mortality-2024\\output\\results_scenario0.txt")
