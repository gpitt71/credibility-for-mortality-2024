...
pop_df <- data.frame("birth"=rep(ages,each=N),
                     "death"=rep(rep(NA,N),each=age_classes),
...
datahat <- structure(list(Dxt = Dtot - D1 - D2, 
                          Ext = Etot - E1 - E2, 
                          ages = ages.fit,
                          years = years.fit,
                          type = 'central', 
...
plot(C1)
library(ggplot2)
ggplot(data.frame(y=D1[30,]/(E1*muxt_hat)[30,],
                  x=1:dim(D1)[2]),
       aes(x=x,
           y=y))+
  geom_point()+
  geom_hline(yintercept = C1[30])+
  geom_hline(yintercept = C1[30]+sqrt(varthetax_1[30]))+
  geom_hline(yintercept = C1[30]-sqrt(varthetax_1[30]))
